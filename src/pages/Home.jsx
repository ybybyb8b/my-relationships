        ...
        {sortedFriends.length === 0 ? (
          ...
          sortedFriends.map(friend => (
            <div key={friend.id} className="relative group">
              ...
              {friend.daysDiff >= 0 && (
                <div className={cn(
                  "absolute top-4 right-4 px-2 py-0.5 rounded-full text-[10px] font-bold shadow-md backdrop-blur-md flex items-center gap-1 z-10 transition-all ring-1 ring-black/5 dark:ring-white/10",

                  friend.status === 'overdue' 
                    ? "bg-red-50/95 dark:bg-red-500/20 text-red-700 dark:text-red-300" 
                    : friend.status === 'safe'
                      ? "bg-green-50/95 dark:bg-green-500/20 text-green-800 dark:text-green-300" 
                      : "bg-white/90 dark:bg-zinc-800/90 text-gray-800 dark:text-gray-100" 
                )}>
                  <div className={cn(
